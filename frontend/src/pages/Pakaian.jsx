...
const Pakaian = () => {
  ...
  const activeCategory = "pakaian"; // Kategori tetap makanan

  // Fungsi untuk mengambil data dari API
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const url = `http://localhost:3000/api/items?categoryId=1&take=30&filterBy=popular`;
    ...
    <main className="mt-10">
      ...
      <div className="flex justify-between items-center mt-20 p-6 relative">
        <h1 className="text-xl font-bold flex items-center">
          <i className="fas fa-gem mr-2"></i> Pakaian Khas Jawa Tengah
        </h1>
        <button
